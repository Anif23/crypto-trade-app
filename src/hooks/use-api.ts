...
export const queryKeys = {
  ...
  history: (id: string, interval: CandleInterval) =>
    ["history", id, interval] as const,
...
export function useHistory(
  ...
  return useQuery<HistoryResponse>({
    queryKey: assetId
      ? queryKeys.history(assetId, interval)
      : ["history", "none", interval],

    enabled: !!assetId,

    queryFn: async () => {
      const resp = await callEdge({
        action: "history",
        asset: assetId!,
        interval,
      });
      ...
      if (!resp.ok) {
        ...
        throw new Error("market_data_failed");
      }
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("holdings")
        .select("*, crypto_assets:symbol,name,image_url,current_price")
        .gt("quantity", 0);
      if (error) throw new Error(error.message);
      return data;
    },
...
export function useOrders(filters?: {
  ...
  return useQuery<{ rows: Order[]; total: number }>({
    ...
    queryFn: async () => {
      let q = supabase
        .from("orders")
        .select("*, crypto_assets!asset_id(symbol,name,image_url)", {
          count: "exact",
        })
        .order("created_at", { ascending: false });
      if (filters?.side && filters.side !== "ALL")
        q = q.eq("side", filters.side);
      ...
      const filtered = filters?.search
        ? rows.filter(
            (r) =>
              (r.crypto_assets?.symbol ?? "")
                .toLowerCase()
                .includes(filters.search!.toLowerCase()) ||
              (r.crypto_assets?.name ?? "")
                .toLowerCase()
                .includes(filters.search!.toLowerCase()),
          )
        : rows;
  ...
  return useMutation({
    ...
    onMutate: async ({ assetId, add }) => {
      ...
      qc.setQueryData(queryKeys.watchlist, (old: unknown) => {
        ...
        if (add) {
          if (old.some((w: { asset_id?: string }) => w.asset_id === assetId))
            return old;
    ...
    onError: (err: Error, _vars, ctx) => {
      ...
      toast({
        ...
        description: friendlyError(
          err.message || "Could not update the watchlist.",
        ),
        variant: "destructive",
      });
    },
